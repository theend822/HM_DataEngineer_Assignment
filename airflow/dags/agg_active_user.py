...
with DAG(
    ...
    wait_for_dim_users = ExternalTaskSensor(
        ...
        execution_date_fn=get_most_recent_dag_run('dim_users'),
        timeout=600,
        poke_interval=30
    )
    ...
    dbt_build_agg_active_user = BashOperator(
        ...
        bash_command="docker exec heymax_assignment-dbt-1 dbt run --models agg_active_user",
    )

    dbt_test_agg_active_user = BashOperator(
        task_id="dbt_test_agg_active_user",
        bash_command="docker exec heymax_assignment-dbt-1 dbt test --models agg_active_user",
    )
