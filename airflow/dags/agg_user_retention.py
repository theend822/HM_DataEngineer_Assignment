...
with DAG(
    ...
    dbt_build_agg_user_retention = BashOperator(
        ...
        bash_command="docker exec heymax_assignment-dbt-1 dbt run --models agg_user_retention",
    )

    dbt_test_agg_user_retention = BashOperator(
        task_id="dbt_test_agg_user_retention",
        bash_command="docker exec heymax_assignment-dbt-1 dbt test --models agg_user_retention",
    )
